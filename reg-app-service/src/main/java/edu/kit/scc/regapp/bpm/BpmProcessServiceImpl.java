...
public class BpmProcessServiceImpl implements BpmProcessService {
    ...
    @Override
	public void replaceAllInRules(String regex, String replacement) {
		List<BusinessRuleEntity> ruleList = ruleDao.findAll();
		
		for (BusinessRuleEntity rule : ruleList) {
			rule.setRule(
					rule.getRule().replaceAll(regex, replacement));
		}
    }
