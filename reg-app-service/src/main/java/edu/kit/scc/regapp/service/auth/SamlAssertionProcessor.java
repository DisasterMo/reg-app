...
import edu.kit.scc.regapp.entity.SamlMetadataEntity;
import edu.kit.scc.regapp.entity.SamlSpConfigurationEntity;
import edu.kit.scc.regapp.exc.NoAssertionException;
...
public class SamlAssertionProcessor {
	...
	@Inject
	private SamlResponseValidatior samlValidatior;
	...
	public Assertion processSamlResponse(Response samlResponse, SamlMetadataEntity idpEntity, 
			EntityDescriptor idpEntityDescriptor, SamlSpConfigurationEntity spEntity, boolean checkSignature) 
					throws IOException, DecryptionException, SamlAuthenticationException {
		samlValidatior.verifyStatus(samlResponse);
		samlValidatior.verifyIssuer(idpEntity, samlResponse);
		samlValidatior.verifyExpiration(samlResponse, 1000L * 60L * 10L);
		...
		if (checkSignature) {
			try {
				...
				samlValidatior.validateIdpSignature(samlResponse, samlResponse.getIssuer(), idpEntityDescriptor);
		...
		if (checkSignature) {
			if (! responseSignatureValid) {
				...
				samlValidatior.validateIdpSignature(assertion, assertion.getIssuer(), idpEntityDescriptor);
